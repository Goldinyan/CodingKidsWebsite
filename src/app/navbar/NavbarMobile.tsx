...
export default function NavbarMobile({ setOpen }: NavbarMobileProps) {
	...
	const navItems = useMemo(() => {
		const items: { label: string; href: string; Icon: LucideIcon }[] = [
			...
			{ label: "Ressourcen", href: "/ressources", Icon: BookOpen }
		];
					...
					{navItems.map((it) => (
						<motion.div
							...
							className={`flex w-full items-center px-4 py-3.5 border rounded-xl transition-all duration-200 cursor-pointer group ${theme === "dark"
								? "border-white/05 bg-white/2 hover:bg-white/5 hover:border-white/15"
						...
						>
							<div className="relative mr-4 flex items-center justify-center">
								...
								{it.label === "Kontakt" && unreadMessagesCount > 0 && (
									<span
										className={`absolute rounded-full -top-2 -right-2 text-[10px] font-bold px-1.5 py-0.5 min-w-4.5 text-center ${theme === "dark"
											? "bg-white text-black"
