...
export function DeleteUserDialog(props: {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        className={`max-w-md border p-6 transition-colors duration-200 focus:outline-none ${radiusClass} ${isDark
      ...
      >
        <DialogHeader className="mb-4">
          ...
          >
            Soll die Berechtigung für{" "}
            <span
              className={
                isDark ? "text-white font-bold" : "text-slate-900 font-bold"
              }
            >
              {userName ? `"${userName}"` : "Diesen Nutzer"}
            </span>{" "}
            permanent aus der System-Matrix entfernt werden?
          </DialogDescription>
        </DialogHeader>

        <div
          className={`space-y-1 p-3 border border-dashed font-['JetBrains_Mono'] text-[10px] tracking-wider uppercase mb-4 ${radiusClass} ${isDark
              ? "bg-red-950/20 border-red-900/50 text-red-400"
              : "bg-red-50 border-red-200 text-red-700"
            }`}
        >
          <div>
            WARNUNG: Diese Operation überschreibt Datenstrukturen irreversibel.
          </div>
          <div className="opacity-60">USER_ID: {userId}</div>
        </div>
