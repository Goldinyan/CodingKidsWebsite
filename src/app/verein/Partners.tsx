...
import cubes_logo from "@/public/cubes-logo.png";
import loris_logo from "@/public/Loris-logo.webp";
import sparkasse_logo from "@/public/sparkasse.webp";

const partners = [
	{ name: "CUBES Wesel", role: "Veranstaltungsort", pic: cubes_logo },
	{ name: "Niederrheinische Sparkasse RheinLippe", role: "Förderer", pic: sparkasse_logo },
	{ name: "Loris Sensory Care GmbH", role: "Partner", pic: loris_logo },
] as const;
			...
			<div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
				...
				>
					...
					>
						...
						{/* 3 Partner gleichmäßig nebeneinander in einer Reihe */}
...
function PartnerCard({ partner, className = "" }: { partner: Partner; className?: string }) {
	...
	return (
		<div
			className={`${className} p-4 border group flex flex-col items-center text-center justify-between transition-all duration-150 ${radiusClass} ${theme === "dark"
				? "bg-zinc-950/40 border-zinc-900 hover:border-[#4ADE80]"
				: "bg-slate-50 border-slate-200 hover:border-green-600"
				}`}
		>
			{/* Zentrierter Container für das Logo: Nimmt die volle Breite der Karte, aber max h-12 */}
			...
			{/* Name und Rolle unterhalb des Logos */}
			<div className="flex flex-col items-center justify-center min-w-0 w-full">
				<h3
					className={`text-xs font-black font-['Familjen_Grotesk'] tracking-wide uppercase line-clamp-2 leading-tight ${theme === "dark" ? "text-white" : "text-slate-900"
						}`}
				>
					{name}
				</h3>
