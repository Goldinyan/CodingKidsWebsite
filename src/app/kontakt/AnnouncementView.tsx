...
export default function AnnouncementView() {
	...
	const handleMarkAsRead = async (announcementUid: string) => {
		...
		try {
			...
			// we dont refresh we just change the readBy because it worked

			rawAnnouncements.forEach((an) => {
				if (an.uid === announcementUid) {
					an.readBy = [...(an.readBy || []), user.uid];
				}
			});
	...
	return (
		<div className="w-full font-['DM_Sans']">
			<div className="text-left flex items-center justify-between  pb-4  border-zinc-200 dark:border-zinc-800">
				...
				<span
					className={`font-['JetBrains_Mono'] md:flex hidden text-xs font-bold px-3 py-1 border ${badgeRadiusClass} ${theme === "dark" ? "bg-zinc-950 text-zinc-400 border-zinc-800" : "bg-slate-50 text-slate-600 border-slate-200"}`}
				>
										...
										{an.readBy && an.readBy.length > 0 ? (
											<>
												{an.readBy.includes(user?.uid || "") ?
													...
													/> : <X className={`w-3.5 h-3.5 ${theme === "dark" ? "text-red-400" : "text-red-600"}`} />}

												<span>
													{an.readBy.includes(user?.uid || "") ? "Gelesen" : "Noch nicht gelesen"}
												</span>
											</>
										) : (
											<span>Ungelesen</span>
